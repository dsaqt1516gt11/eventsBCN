...
public interface EventDAOQuery {
    ...
    public final static String GET_EVENTS_BY_CATEGORY = "select hex(e.id) as id, e.title,e.description,e.date,e.photo,e.category,hex(e.companyid) as companyid, e.last_modified, e.creation_timestamp from events e where category = ? and date > CAST(CURRENT_TIMESTAMP AS DATE)";
